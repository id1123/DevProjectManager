...
function normalizeDashboard(value: DashboardData): DashboardData {
  return {
    ...
    settings: { ...emptyDashboard.settings, ...(value.settings ?? {}), defaultIdeIds: value.settings?.defaultIdeIds ?? {} },
  };
}
...
export default function MainApp() {
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        {loading ? (
          ...
          <ProjectDetail
            ...
            onReorder={(moduleIds) => run(() => api.reorderModules(selectedProject.id, moduleIds), "项目顺序已更新")}
          />
        ) : (
          <>
      ...
      {moduleEditor && <ModuleModal project={moduleEditor.project} module={moduleEditor.module} definitions={dashboard.ideDefinitions} installations={dashboard.ideInstallations} defaultIdeIds={dashboard.settings.defaultIdeIds} tagSuggestions={allTags} onClose={() => setModuleEditor(null)} onSave={(input) => run(() => api.saveModule(input), input.id ? "项目已更新" : "项目已添加").then((saved) => { if (saved) setModuleEditor(null); })} />}
      {settingsOpen && <Modal title="设置" subtitle="调整外观、快捷键、IDE 和本地配置。" onClose={() => setSettingsOpen(false)} wide>
        ...
        <div style={{ display: settingsTab === "general" ? undefined : "none" }}><SettingsPage settings={dashboard.settings} ideDefinitions={dashboard.ideDefinitions} onSave={async (settings) => { const saved = await run(async () => { if (settings.globalShortcut !== dashboard.settings.globalShortcut) await api.setGlobalShortcut(settings.globalShortcut); await api.saveSettings(settings); }, "设置已保存"); if (saved) setSettingsOpen(false); }} onImport={async () => { const path = await open({ multiple: false, directory: false, filters: [{ name: "Project Hub 配置", extensions: ["json"] }] }); if (typeof path === "string") await run(() => api.importConfig(path), "配置已导入，失效路径已保留并标记"); }} onExport={async () => { const path = await save({ defaultPath: "project-hub-config.json", filters: [{ name: "Project Hub 配置", extensions: ["json"] }] }); if (path) await run(() => api.exportConfig(path), "配置已导出"); }} /></div>
...
function ProjectDetail({ project, dashboard, onBack, onEditProject, onAddModule, onEditModule, onLaunch, onReveal, onDeleteModule, onReorder }: { project: Project; dashboard: DashboardData; onBack: () => void; onEditProject: () => void; onAddModule: () => void; onEditModule: (module: ProjectModule) => void; onLaunch: (module: ProjectModule, ideId?: string) => void; onReveal: (module: ProjectModule) => void; onDeleteModule: (module: ProjectModule) => void; onReorder: (moduleIds: string[]) => Promise<boolean> }) {
  const [orderedIds, setOrderedIds] = useState(() => project.modules.map((module) => module.id));
  const [draggingId, setDraggingId] = useState<string | null>(null);
  const [dropPosition, setDropPosition] = useState<{ id: string; after: boolean } | null>(null);
  const [sortingBusy, setSortingBusy] = useState(false);
  const dragSource = useRef<{ id: string; pointerId: number; startX: number; startY: number; started: boolean } | null>(null);
  const moduleListRef = useRef<HTMLElement | null>(null);
  useEffect(() => setOrderedIds(project.modules.map((module) => module.id)), [project.id, project.modules]);
  const orderedModules = orderedIds.map((id) => project.modules.find((module) => module.id === id)).filter((module): module is ProjectModule => !!module);
  function clearDrag() { dragSource.current = null; setDraggingId(null); setDropPosition(null); }
  function findDropPosition(x: number, y: number) {
    const list = moduleListRef.current;
    if (!list) return null;
    const bounds = list.getBoundingClientRect();
    if (x < bounds.left || x > bounds.right || y < bounds.top || y > bounds.bottom) return null;
    const rows = Array.from(list.querySelectorAll<HTMLElement>(".module-row"));
    for (const row of rows) {
      const rowBounds = row.getBoundingClientRect();
      if (y < rowBounds.top + rowBounds.height / 2) return { id: row.dataset.moduleId!, after: false };
    }
    return rows.length ? { id: rows[rows.length - 1].dataset.moduleId!, after: true } : null;
  }
  async function moveModule(sourceId: string, targetId: string, after: boolean) {
    if (sortingBusy || sourceId === targetId) return;
    const next = orderedIds.filter((id) => id !== sourceId);
    const targetIndex = next.indexOf(targetId);
    if (targetIndex < 0 || !orderedIds.includes(sourceId)) return;
    next.splice(targetIndex + (after ? 1 : 0), 0, sourceId);
    if (next.every((id, index) => id === orderedIds[index])) return;
    setSortingBusy(true);
    setOrderedIds(next);
    if (!await onReorder(next)) setOrderedIds(orderedIds);
  ...
  return <>
    ...
    <div className="section-heading"><div><h2>项目</h2><p>{project.modules.length} 个已配置项目{project.modules.length > 1 && " · 拖动左侧手柄调整顺序"}</p></div></div>
    <section className="module-list" ref={moduleListRef}>{orderedModules.length ? orderedModules.map((module) => {
      ...
      return <article className={`module-row ${draggingId === module.id ? "dragging" : ""} ${dropPosition?.id === module.id ? dropPosition.after ? "drop-after" : "drop-before" : ""}`} data-module-id={module.id} key={module.id}>
        <span className="module-drag-handle" role="button" tabIndex={0} title={`拖动调整 ${module.name} 的顺序`} aria-label={`拖动调整 ${module.name} 的顺序，可用上下方向键移动`} onPointerDown={(event) => { if (event.button !== 0 || !event.isPrimary || sortingBusy) return; event.preventDefault(); event.currentTarget.setPointerCapture(event.pointerId); dragSource.current = { id: module.id, pointerId: event.pointerId, startX: event.clientX, startY: event.clientY, started: false }; }} onPointerMove={(event) => { const source = dragSource.current; if (!source || source.pointerId !== event.pointerId) return; if (!source.started) { if (Math.hypot(event.clientX - source.startX, event.clientY - source.startY) < 5) return; source.started = true; setDraggingId(source.id); } const target = findDropPosition(event.clientX, event.clientY); setDropPosition(target?.id === source.id ? null : target); }} onPointerUp={(event) => { const source = dragSource.current; if (!source || source.pointerId !== event.pointerId) return; const target = source.started ? findDropPosition(event.clientX, event.clientY) : null; clearDrag(); if (target) void moveModule(source.id, target.id, target.after); }} onPointerCancel={clearDrag} onLostPointerCapture={clearDrag} onKeyDown={(event) => { const index = orderedIds.indexOf(module.id); if (event.key === "ArrowUp" && index > 0) { event.preventDefault(); void moveModule(module.id, orderedIds[index - 1], false); } else if (event.key === "ArrowDown" && index < orderedIds.length - 1) { event.preventDefault(); void moveModule(module.id, orderedIds[index + 1], true); } }}><GripVertical size={17} /></span>
        ...
        <div className="module-actions">
          <button className="secondary-button small reveal-directory-button" onClick={() => onReveal(module)}><FolderOpen size={16} />打开目录</button>
          <button className="secondary-button small module-edit-action" title={`编辑 ${module.name}`} onClick={() => onEditModule(module)}><Pencil size={15} />编辑</button>
          <button className="secondary-button small module-delete-action" title={`删除 ${module.name}`} onClick={() => onDeleteModule(module)}><Trash2 size={15} />删除</button>
          <div className="split-action"><button className="secondary-button module-launch-button small" disabled={invalid || !ide || !installation || !["available", "installed"].includes(installation.status)} onClick={() => onLaunch(module)}><ExternalLink size={16} />用 {ide?.name ?? "IDE"} 打开</button><details className="action-details"><summary aria-label="更多项目操作" title="更多打开方式"><MoreHorizontal size={17} /></summary><div className="action-menu" onClick={(event) => { event.currentTarget.closest("details")!.open = false; }}><span className="action-menu-label">打开方式</span><button className="reveal-menu-action" onClick={() => onReveal(module)}>打开项目目录</button>{dashboard.ideDefinitions.filter((item) => supportsPath(item, module.path?.pathKind ?? "directory")).map((item) => <button key={item.id} onClick={() => onLaunch(module, item.id)}>临时使用 {item.name}</button>)}</div></details></div>
        </div>
      </article>;
...
function SettingsPage({ settings, ideDefinitions, onSave, onImport, onExport }: { settings: AppSettings; ideDefinitions: IdeDefinition[]; onSave: (settings: AppSettings) => void; onImport: () => void; onExport: () => void }) {
  ...
  const defaultTypes = [{ type: "api", label: "API", pathKind: "file" }, { type: "web", label: "WEB", pathKind: "directory" }, { type: "app", label: "APP", pathKind: "directory" }] as const;
  function setDefaultIde(type: "api" | "web" | "app", ideId: string) {
    const defaultIdeIds = { ...draft.defaultIdeIds };
    if (ideId) defaultIdeIds[type] = ideId;
    else delete defaultIdeIds[type];
    setDraft({ ...draft, defaultIdeIds });
  }
...
function ModuleModal({ project, module, definitions, installations, defaultIdeIds, tagSuggestions, onClose, onSave }: { project: Project; module?: ProjectModule; definitions: IdeDefinition[]; installations: DashboardData["ideInstallations"]; defaultIdeIds: AppSettings["defaultIdeIds"]; tagSuggestions: string[]; onClose: () => void; onSave: (value: ModuleInput) => void }) {
  function defaultIde(type: ModuleType, kind: PathKind) {
    const id = type === "other" ? undefined : defaultIdeIds[type];
    return definitions.some((ide) => ide.id === id && ide.supportedPlatforms.includes(currentPlatform()) && supportsPath(ide, kind)) ? id ?? "" : "";
  }
  const [name, setName] = useState(module?.name ?? ""); const [moduleType, setModuleType] = useState<ModuleType>(module?.moduleType ?? "api"); const [pathKind, setPathKind] = useState<PathKind>(module?.path?.pathKind ?? (moduleType === "api" ? "file" : "directory")); const [path, setPath] = useState(module?.path?.path ?? ""); const [ideId, setIdeId] = useState(module ? module.ideId ?? "" : defaultIde("api", "file")); const [description, setDescription] = useState(module?.description ?? ""); const [tags, setTags] = useState(module?.tags ?? [module?.moduleType ?? "api"]); const [favorite, setFavorite] = useState(module?.isFavorite ?? false); const [linkedModuleIds, setLinkedModuleIds] = useState(module?.linkedModuleIds ?? []);
...
function Modal({ title, subtitle, children, onClose, wide = false, nested = false }: { title: string; subtitle?: string; children: React.ReactNode; onClose: () => void; wide?: boolean; nested?: boolean }) { return createPortal(<div className={`modal-backdrop ${nested ? "nested" : ""}`} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className={`modal ${wide ? "wide" : ""}`}><header><div><h2>{title}</h2>{subtitle && <p>{subtitle}</p>}</div><button className="icon-button" onClick={onClose}><X size={19} /></button></header><div className="modal-body">{children}</div></section></div>, document.body); }
