...
export default function MainApp() {
  ...
  const [compactMode, setCompactMode] = useState(() => window.localStorage.getItem("project-hub:list-density") === "compact");
        ...
        {loading ? (
          ...
          <>
            <header className="page-header">
              ...
              <button className="primary-button" onClick={() => setProjectEditor("new")}><Plus size={18} />新建业务项目</button>
            </header>
            <div className="toolbar">
              ...
              {view === "all" && <button className={`secondary-button ${compactMode ? "active" : ""}`} onClick={() => setCompactMode((value) => { const next = !value; window.localStorage.setItem("project-hub:list-density", next ? "compact" : "detailed"); return next; })}><Rows3 size={17} />{compactMode ? "详细模式" : "精简模式"}</button>}
              <button className="secondary-button" onClick={() => void refresh()}><RefreshCw size={17} />刷新</button>
            </div>
            ...
            {visibleModules.length || (view === "all" && visibleProjects.length) ? (
              ...
              <div className="empty-state"><div><Boxes size={28} /></div><h2>{query || favoriteOnly || selectedTags.length ? "没有匹配结果" : view === "recent" ? "还没有最近打开的项目" : "还没有业务项目"}</h2><p>{query || favoriteOnly || selectedTags.length ? "换个关键词，或清空筛选条件。" : view === "recent" ? "从全部项目中打开项目后，会显示在这里。" : "先创建一个业务项目，再添加 API、Web 或 App 项目。"}</p>{view === "all" && !query && !favoriteOnly && selectedTags.length === 0 && <button className="primary-button" onClick={() => setProjectEditor("new")}><Plus size={18} />创建第一个业务项目</button>}</div>
            )}
          </>
        )}
      </main>

      {projectEditor && <ProjectModal project={projectEditor === "new" ? undefined : projectEditor} tagSuggestions={allTags} onClose={() => setProjectEditor(null)} onSave={(input) => run(() => api.saveProject(input), input.id ? "项目已更新" : "项目已创建").then(() => setProjectEditor(null))} />}
...
function ProjectCard({ project, compact, definitions, installations, onOpen, onLaunch, onAddModule, onEditModule, onEdit, onDelete }: { project: Project; compact: boolean; definitions: IdeDefinition[]; installations: DashboardData["ideInstallations"]; onOpen: () => void; onLaunch: (module: ProjectModule) => void; onAddModule: () => void; onEditModule: (module: ProjectModule) => void; onEdit: () => void; onDelete: () => void }) {
  return <article className="project-card">
    ...
    <div className={`card-module-list ${compact ? "compact-list" : ""}`}>{project.modules.map((module) => {
      ...
      if (compact) return <article key={module.id} className="compact-module-card" role="button" tabIndex={0} title={`用 ${ide?.name ?? "默认 IDE"} 打开`} onClick={() => onLaunch(module)} onKeyDown={(event) => { if (event.target === event.currentTarget && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); void onLaunch(module); } }}><strong>{module.name}</strong><button className="icon-button compact" title="编辑项目" onClick={(event) => { event.stopPropagation(); onEditModule(module); }}><Pencil size={13} /></button></article>;
      return <article key={module.id} className="card-module-card" role="button" tabIndex={0} title={`用 ${ide?.name ?? "默认 IDE"} 打开`} onClick={() => onLaunch(module)} onKeyDown={(event) => { if (event.target === event.currentTarget && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); void onLaunch(module); } }}>
        <div className="card-module-card-top"><span className={`module-icon ${moduleTypeTone[module.moduleType]}`}><FileCode2 size={19} /></span><span className={`type-badge ${moduleTypeTone[module.moduleType]}`}>{moduleTypeLabel[module.moduleType]}</span>{module.isFavorite && <Star size={13} fill="currentColor" />}<button className="icon-button" title="编辑项目" onClick={(event) => { event.stopPropagation(); onEditModule(module); }}><Pencil size={14} /></button></div>
    ...
    })}<button className={`card-add-module ${compact ? "compact-add" : ""}`} title="添加项目" onClick={onAddModule}><Plus size={compact ? 16 : 24} /><span>添加项目</span></button></div>
...
function RecentProjectList({ items, definitions, installations, onLaunch, onEdit }: { items: { project: Project; module: ProjectModule }[]; definitions: IdeDefinition[]; installations: DashboardData["ideInstallations"]; onLaunch: (module: ProjectModule) => void; onEdit: (project: Project, module: ProjectModule) => void }) {
  return <section className="recent-project-list">{items.map(({ project, module }) => {
    const ide = ideForModule(module.ideId, definitions);
    const installation = ide ? ideInstallation(ide, installations) : undefined;
    const ready = module.path?.validationStatus === "valid" && !!ide && !!installation && ["available", "installed"].includes(installation.status);
    return <article key={module.id} className="recent-project-row" role="button" tabIndex={0} onClick={() => onLaunch(module)} onKeyDown={(event) => { if (event.target === event.currentTarget && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); onLaunch(module); } }}>
      ...
      <ExternalLink size={16} className="recent-project-open" />
    </article>;
  })}</section>;
}
  ...
  return <>
    ...
    <section className="module-list">{project.modules.length ? project.modules.map((module) => {
      ...
      return <article className="module-row" key={module.id}>
        ...
        <div className="module-main"><div className="module-name"><h3>{module.name}</h3><span className={`type-badge ${moduleTypeTone[module.moduleType]}`}>{moduleTypeLabel[module.moduleType]}</span>{module.isFavorite && <Star size={14} fill="currentColor" />}</div>{module.tags.length > 0 && <div className="entity-tags compact">{module.tags.map((tag) => <span key={tag}>{tag}</span>)}</div>}<p className={invalid ? "path invalid" : "path"}>{displayPath(module.path?.path ?? "")}{invalid && <span> · 路径不存在</span>}</p><span className="module-meta">{ide?.name ?? "未指定 IDE"} · {relativeTime(module.lastOpenedAt)}</span></div>
        <div className="module-actions"><button className="secondary-button small" onClick={() => onReveal(module)}><FolderOpen size={16} />定位</button><div className="split-action"><button className="primary-button small" disabled={invalid || !ide || !installation || !["available", "installed"].includes(installation.status)} onClick={() => onLaunch(module)}><ExternalLink size={16} />用 {ide?.name ?? "IDE"} 打开</button><details><summary><MoreHorizontal size={17} /></summary><div className="action-menu">{dashboard.ideDefinitions.filter((item) => supportsPath(item, module.path?.pathKind ?? "directory")).map((item) => <button key={item.id} onClick={() => onLaunch(module, item.id)}>临时使用 {item.name}</button>)}<hr /><button onClick={() => onEditModule(module)}>编辑配置</button><button className="danger-text" onClick={() => onDeleteModule(module)}>删除配置</button></div></details></div></div>
      </article>;
...
function IdeManager({ dashboard, onRefresh, onSave, onDelete }: { dashboard: DashboardData; onRefresh: () => void; onSave: (definition: SaveIdeDefinitionInput, installation: Omit<SaveIdeInput, "ideId">) => Promise<void>; onDelete: (ide: IdeDefinition) => void }) {
  ...
  return <>
    ...
    {editing && <IdeEditor ide={editing === "new" ? undefined : editing} installation={editing === "new" ? undefined : ideInstallation(editing, dashboard.ideInstallations)} onClose={() => setEditing(null)} onSave={async (definition, installation) => { await onSave(definition, installation); setEditing(null); }} />}
  </>;
}
...
function ProjectModal({ project, tagSuggestions, onClose, onSave }: { project?: Project; tagSuggestions: string[]; onClose: () => void; onSave: (value: ProjectInput) => void }) {
  ...
  return <Modal title={project ? "编辑业务项目" : "新建业务项目"} subtitle="业务项目用于组织多个项目，本身不对应代码目录。" onClose={onClose}><form className="form-stack" onSubmit={(event) => { event.preventDefault(); onSave({ id: project?.id, name: name.trim(), description: description.trim(), tags, color, isFavorite: favorite, sortOrder: project?.sortOrder ?? 0 }); }}><label>项目名称<input autoFocus required value={name} onChange={(event) => setName(event.target.value)} placeholder="输入项目名称" /></label><label>描述（可选）<textarea value={description ?? ""} onChange={(event) => setDescription(event.target.value)} placeholder="补充项目说明" /></label><TagEditor value={tags} suggestions={tagSuggestions} onChange={setTags} /><label>项目颜色<div className="color-picker">{colors.map((item) => <button type="button" aria-label={item} key={item} className={color === item ? "selected" : ""} style={{ background: item }} onClick={() => setColor(item)} />)}</div></label><label className="checkbox-row"><input type="checkbox" checked={favorite} onChange={(event) => setFavorite(event.target.checked)} /><span><strong>收藏项目</strong><small>在收藏筛选和快速启动中优先展示</small></span></label><div className="modal-actions"><button type="button" className="secondary-button" onClick={onClose}>取消</button><button className="primary-button" disabled={!name.trim()}>{project ? "保存更改" : "创建项目"}</button></div></form></Modal>;
}
...
function ModuleModal({ project, module, definitions, installations, tagSuggestions, onClose, onSave }: { project: Project; module?: ProjectModule; definitions: IdeDefinition[]; installations: DashboardData["ideInstallations"]; tagSuggestions: string[]; onClose: () => void; onSave: (value: ModuleInput) => void }) {
  ...
  return <Modal title={module ? "编辑项目" : `给“${project.name}”添加项目`} subtitle="文件和文件夹会按原始路径保存，不会复制或修改代码。" onClose={onClose} wide><form className="form-stack" onSubmit={(event) => { event.preventDefault(); const selectedIde = definitions.find((ide) => ide.id === ideId); const argumentTemplate = module?.ideId === ideId ? module.argumentTemplate : selectedIde?.defaultArgumentTemplate ?? ["{path}"]; onSave({ id: module?.id, projectId: project.id, name: name.trim(), moduleType, description: description?.trim(), tags, path: { path: path.trim(), pathKind }, ideId: ideId || null, argumentTemplate, isFavorite: favorite, sortOrder: module?.sortOrder ?? project.modules.length }); }}><div className="form-grid"><label>项目名称<input autoFocus required value={name} onChange={(event) => setName(event.target.value)} placeholder="输入项目名称" /></label><label>类型<select value={moduleType} onChange={(event) => { const value = event.target.value as ModuleType; if (!module) setTags((current) => { const withoutPreviousType = current.filter((tag) => tag.toLocaleLowerCase() !== moduleType); return withoutPreviousType.some((tag) => tag.toLocaleLowerCase() === value) ? withoutPreviousType : [...withoutPreviousType, value]; }); setModuleType(value); if (!module) setPathKind(value === "api" ? "file" : "directory"); }}>{Object.entries(moduleTypeLabel).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label></div><label>路径类型<div className="segmented"><button type="button" className={pathKind === "file" ? "active" : ""} onClick={() => { setPathKind("file"); setIdeId(""); }}>文件</button><button type="button" className={pathKind === "directory" ? "active" : ""} onClick={() => { setPathKind("directory"); setIdeId(""); }}>文件夹</button></div></label><label>本地路径<div className="path-picker"><input required value={path} onChange={(event) => setPath(event.target.value)} placeholder={pathKind === "file" ? "选择项目文件" : "选择项目根目录"} /><button type="button" className="secondary-button" onClick={pickPath}><FolderOpen size={17} />选择</button></div></label><label>默认 IDE<select required value={ideId ?? ""} onChange={(event) => setIdeId(event.target.value)}><option value="">选择用于打开该项目的 IDE</option>{availableIdes.map((ide) => { const status = ideInstallation(ide, installations)?.status; const installed = status === "available" || status === "installed"; return <option key={ide.id} value={ide.id}>{ide.name}{installed ? "" : "（未检测到）"}</option>; })}</select><small className="field-help">未检测到的 IDE 仍可保存，之后可到 IDE 管理中重新配置。</small></label><label>描述（可选）<textarea value={description ?? ""} onChange={(event) => setDescription(event.target.value)} placeholder="补充项目说明" /></label><TagEditor value={tags} suggestions={tagSuggestions} onChange={setTags} /><label className="checkbox-row"><input type="checkbox" checked={favorite} onChange={(event) => setFavorite(event.target.checked)} /><span><strong>收藏项目</strong><small>在收藏筛选和快速启动中优先显示</small></span></label><div className="modal-actions"><button type="button" className="secondary-button" onClick={onClose}>取消</button><button className="primary-button" disabled={!name.trim() || !path.trim() || !ideId}>{module ? "保存更改" : "添加项目"}</button></div></form></Modal>;
}
